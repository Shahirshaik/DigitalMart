...
export default async function MyCoursesPage() {
  ...
  let coursesQuery = supabase
    .from("courses").select("*, seller:users!courses_seller_id_fkey(full_name)")
    .order("created_at", { ascending: false });
  if (!isAdmin) coursesQuery = coursesQuery.eq("seller_id", user.id);
      ...
      <main className="flex-1 bg-gray-50">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 py-10">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{isAdmin ? "All Courses" : "My Courses"}</h1>
              <p className="text-sm text-gray-500">
                {isAdmin ? "Every course on the platform." : "Share knowledge and guidance — paid or free (set price to 0)."}
              </p>
            </div>
                ...
                <div key={c.id} className="card p-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {isAdmin && <span className="text-xs text-gray-400">· {(c.seller as any)?.full_name}</span>}
                    </div>
                  </div>
